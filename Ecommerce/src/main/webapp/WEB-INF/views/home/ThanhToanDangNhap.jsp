...
<body>
	<c:url value="/them-dat-hang" var="urlAdd"/>
					...
					<s:form action="${urlAdd }" method="post" id="FormRegister"
						...
						<div class="checkbox-form mb-sm-40">
							<table class="table table-hover">
								<thead>
									<tr>
										<th style="border: none;"><h4 style="font-size: 25px">THÔNG TIN ĐẶT HÀNG 
										<a style="float: right;" href="javascript:void(0)">
										<i class="fa fa-pencil-square-o" id="tooltip" style="color: #C0C0C0;" >
											<span id="tooltiptext">Thêm địa chỉ mới</span>
										</i></a></h4></th>
									</tr>
								</thead>
								<tbody>
									<c:if test="${not empty lstSoDiaChi }">
										<c:forEach var="sdc" items="${lstSoDiaChi }">
											<tr style="border: none;">
												<td colspan="2" style="border: none;">
													<input type="radio" name="diaChi" value="${sdc.id }">
													<label style="font-weight: bold;">${sdc.hoTen }</label>
													<p>${sdc.diaChi } , ${sdc.xa2.ten}, ${sdc.huyen2.ten }, ${sdc.tinh2.ten }</p>
												</td>
											</tr>
										</c:forEach>
									</c:if>
